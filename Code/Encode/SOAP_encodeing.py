...
class ChatEmbeddingConfig:
    ...
    layer_window: int = 0
    layer_mixing: LayerMixing = "avg"
    layer_mixing_tau: float = 1.0
    include_embedding_in_mixing: bool = False
    pooling_method: PoolMethod = "mean"
...
def _mix_layers(
    ...
    if mode == "exp":
        ...
        weights = torch.exp(-distances / max(1e-6, tau))
...
class ChatEmbedder:
    def __init__(self, cfg: ChatEmbeddingConfig):
        ...
        self.evaluation_layer_idx = self.total_layers if cfg.evaluation_layer_idx == -1 else cfg.evaluation_layer_idx
        ...
        model_kwargs["device_map"] = None if cfg.force_device else cfg.device_map
...
"""
        ...
        if mode == "a":
            system_msg_a = "You are an emergency clinician using the SOAP framework for ESI triage (Emergency Severity Index, levels 1–5)."
            user_msg = f"""Below is a short triage note describing a patient's condition:
...
"""
    ...
    def _tokenize_triplet(self, messages_batch: List[List[Dict[str, str]]]):
        ...
        prompts_su = [tokenizer.apply_chat_template(msgs, tokenize=False, add_generation_prompt=False) for msgs in messages_batch]
...
def parse_args():
    ...
    parser.add_argument("--force_device", type=str, default=None)
    parser.add_argument("--add_generation_prompt", action="store_true")
    parser.add_argument("--soap_mode", type=str, default="none", choices=["none", "s", "o", "a", "p"])
