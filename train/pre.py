...
# ==============================
# Models (支持 linear/mlp/gated/bert)
# ==============================
...
class StreamBertEncoder(nn.Module):
    """
    BERT风格 TransformerEncoder，但输入是“向量token序列”
    tokens: [B, L, D]，加可学习pos emb，然后 encoder，再pool
    """
    def __init__(
        self,
        embed_dim: int,
        ...
        enc_layer = nn.TransformerEncoderLayer(
            ...
            activation="gelu",
            batch_first=True,
            norm_first=True,   # 与训练脚本一致（会有nested tensor warning但不影响结果）
        )
...
class BinaryGatedHead(nn.Module):
    ...
    def forward(self, streams: List[torch.Tensor]):
        ...
        fused = torch.sum(weights.unsqueeze(-1) * stack, 1)  # [B,D]
...
class BinaryBertHead(nn.Module):
    ...
    与训练脚本一致：
      - gate 得到 fused token
      - seq = [fused_token] + [stream_tokens]
      - TransformerEncoder -> pool first token
    """
    def __init__(
        self,
        embed_dim: int,
...
def main():
    ...
    # load ckpt1 for norm/stats and stage1 model
    ...
    stage1_model = build_binary_model(
        ...
        gate_use_gumbel=False,  # 推理不要gumbel
    ...
    with torch.no_grad(), open(args.output_path, "w", encoding="utf-8") as f:
        for streams, labels, batch_ids in dl:
            ...
            prob1 = F.softmax(logits1, dim=1)           # [B,2]
            coarse = logits1.argmax(dim=1)              # 0 high / 1 low

            # stage2 high (保持原功能：全量跑一遍以输出权重)
            logits_h, aux_h = stage2_high_model(streams)
            wh = _extract_weights(aux_h)
            pred_h = coral_predict(logits_h, threshold=args.coral_threshold)  # 0/1

            # stage2 low (保持原功能：全量跑一遍以输出权重)
            logits_l, aux_l = stage2_low_model(streams)
            wl = _extract_weights(aux_l)
            pred_l = coral_predict(logits_l, threshold=args.coral_threshold) + 2  # 2/3/4

            final_pred = torch.where(coarse == 0, pred_h, pred_l)  # 0..4
            ...
            for i, _id in enumerate(batch_ids):
                ...
                obj: Dict[str, Any] = {
                    ...
                    "stage1_coarse_pred": int(coarse_list[i]),      # 0=high,1=low
                    "stage1_coarse_prob": prob1_list[i],            # [p(high), p(low)]
