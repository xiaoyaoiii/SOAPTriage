...
def align_streams(feature_files: List[str]) -> Tuple[List[torch.Tensor], torch.Tensor, List[str]]:
    ...
    for s in streams:
        ...
        all_X.append(torch.stack(embs, dim=0))
...
def coral_targets(labels: torch.Tensor, num_classes: int) -> torch.Tensor:
    ...
    return (labels_expanded > thresholds).float()
...
def main():
    ...
    dl_train_s1 = DataLoader(ds_train_s1, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers, collate_fn=collate_fn)
    dl_val_s1 = DataLoader(ds_val_s1, batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers, collate_fn=collate_fn)
    ...
    y_high_train = train_y_all[idx_train_high]
    ...
    dl_train_h = DataLoader(ds_train_h, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers, collate_fn=collate_fn)
    dl_val_h = DataLoader(ds_val_h, batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers, collate_fn=collate_fn)
    ...
    dl_train_l = DataLoader(ds_train_l, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers, collate_fn=collate_fn)
    dl_val_l = DataLoader(ds_val_l, batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers, collate_fn=collate_fn)
